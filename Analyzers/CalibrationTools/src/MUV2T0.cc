...
MUV2T0::MUV2T0(Core::BaseAnalysis *ba) : T0Evaluation(ba, "MUV2") {
    
    // Optimal parameters
    fMinIntegral         = 1000;   // minimal number of entries (excluding underflows, overflows) for fit attempt
    fMinContentMaxBin    = 100.0;  // minimal content of most populated bin for fit attempt
    fFittingRange        = 4.;   // fitting range = [-0.9ns:+0.9ns], i.e. 9 bins of 0.2ns width
    ...
    fMaxDeltaT0          = 1.0;   // max precision of T0 to consider the fit successful
