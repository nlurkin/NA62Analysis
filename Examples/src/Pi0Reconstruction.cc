...
void Pi0Reconstruction::Process(int iEvent, MCSimple &fMCSimple, Event* MCTruthEvent){
	...
	vector<KinePart*> photons, lavCand;
	KinePart *part;
	...
	if(state==kOValid){
		...
		photons.insert(photons.end(), lavCand.begin(), lavCand.end());
		...
		if(photonOrder.size()>=2){
			...
			if( (g1Vol != DetectorAcceptance::kLAV && g1LKr == true)
				...
				pi0.SetInitialEnergy(g1->GetInitialEnergy() + fMCSimple[22][iTrail]->GetInitialEnergy());
				...
				direction.SetMag(fMCSimple[22][iTrail]->GetInitialEnergy());
